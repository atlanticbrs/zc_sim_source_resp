...
# if needed to add more fonts from your system into R, first run:
# font_import(prompt = FALSE)
# then this needs to be run every R session:
loadfonts()
...
# from:
# devtools::install_github("CalvinData/CalvinBayes")
acf_plot <- function (x, lag.max = NULL, type = c("correlation", "covariance", 
    "partial"), plot = FALSE, na.action = na.fail, demean = TRUE, 
    ..., color = "steelblue", fill = "black", alpha = 0.7, data = NULL) 
{
    if (!is.null(data)) {
        x <- eval(substitute(x), data, parent.frame())
    }
    acf_out <- stats::acf(x, lag.max = lag.max, type = type, 
    ...
    fence <- c(-1, 1) * fence
    ggplot() + geom_col(aes(x = lag, y = acf), data = ACF, fill = fill, 
        alpha = alpha) + geom_hline(yintercept = fence, linetype = "dashed", 
        color = color)
}
```
...
Michelot, Théo, et al. "Continuous-time modelling of behavioural responses in animal movement." arXiv preprint arXiv:2212.09574 (2022).
...
The data has undergone pre-processing in Matlab. See files "prep_SDEs.Rmd" and "smoothSDE_prep.m" for details.
...
The numeric values for change in sigma are on the scale of the linear predictor, so can be thought of as being in some kind of relative units.
...
deep_ddeptht_sde$plot_par("diveprop", par_names = "sigma", n_post = 1e4,
              ...
              resp = FALSE, show_CI = "simultaneous") |>
  gf_theme(strip.text.y = element_blank()) |>
  gf_labs(title = 'DTag20_232', y = 'Depth Rate\nDifference Smooth', x = 'Proportion of Dive') |>
  gf_hline(yintercept = 0, color = 'black', size = 1.1)
```
...
We might consider modeling heading directly, or difference in heading, or some other treatment respecting the angular nature of the variable. Michelot et al just used angles with no worries...but perhaps they got lucky. In our case, either one (heading or diff(heading)) includes many "circular moves" (large changes and/or heading going from for example 350 to 10 degrees).
...
deep_head_tsde$plot_par("diveprop", par_names = "sigma", n_post = 1e4,
              ...
              resp = FALSE, show_CI = "simultaneous") |>
  gf_theme(strip.text.y = element_blank()) |>
  gf_labs(title = 'DTag19_218', y = 'Change in Diffusion Parameter\n(Deep Heading)', x = 'Proportion of Dive') |>
  gf_hline(yintercept = 0, color = 'black', size = 1.1)
```
...
deep_head_tsde$plot_par("diveprop", par_names = "sigma", n_post = 1e4,
              ...
              resp = FALSE, show_CI = "simultaneous") |>
  gf_theme(strip.text.y = element_blank()) |>
  gf_labs(title = 'DTag20_232', y = 'Change in Diffusion Parameter\n(Deep Heading)', x = 'Proportion of Dive') |>
  gf_hline(yintercept = 0, color = 'black', size = 1.1)
```
...
shallow_ddepth_sde$plot_par("diveprop", par_names = "sigma", n_post = 1e4,
              ...
              resp = FALSE, show_CI = "simultaneous") |>
  gf_theme(strip.text.y = element_blank()) |>
  gf_labs(title = 'DTag17_234', y = 'Depth Rate\nDifference Smooth', x = 'Proportion of Dive') |>
  gf_hline(yintercept = 0, color = 'black', size = 1.1)
```
...
shallow_head_tsde$plot_par("diveprop", par_names = "sigma", n_post = 1e4,
              ...
              resp = FALSE, show_CI = "simultaneous") |>
  gf_theme(strip.text.y = element_blank()) |>
  gf_labs(title = 'DTag17_234', y = 'Change in Diffusion Parameter\n(Shallow Heading)', x = 'Proportion of Dive') |>
  gf_hline(yintercept = 0, color = 'black', size = 1.1)
```
...
acf_plot(resids_shal_ddepth |>
           na.omit()) |>
  gf_labs(title = "Depth Difference", y = 'Residual ACF', x = 'Lag')

acf_plot(resids_shal_ddeptht |>
           na.omit()) |>
  gf_labs(title = "Depth Difference (t)", y = 'Residual ACF', x = 'Lag')

acf_plot(resids_shal_head |>
           na.omit()) |>
  gf_labs(title = "Heading", y = 'Residual ACF', x = 'Lag')

acf_plot(resids_shal_headt |>
           na.omit()) |>
  gf_labs(title = "Heading (t)", y = 'Residual ACF', x = 'Lag')
```
...
## Figure 8: DTAG Data in 3 Panels
...
zlim <- c(1700,0) # y axis limits for depth plots

echoloc_dot_size <- 1 # size for points showing start/end of echolocation and also RLs
rl_dot_size <- 0.25 * echoloc_dot_size
head_dot_size <- 0.08 # size of points for plotting heading
```

### 25Hz DTAG Data

```{r, prep-25Hz}
# note that the 25 Hz DTAG data used to make this figure is not included in the GitHub repo
# as it is too large. It can be shared on request.
...
### Prepare Depth, Heading, MSA Panels
...
#### Add RLs to Dive Traces on 2nd Y Axis
...
### Combine Depth, Heading, MSA in Column for each Whale

```{r, prep-timeseries-three-whales, fig.width = 7.5, fig.height=9}
...
```{r, fig-8-DTAG-timeseries, fig.width = 7, fig.height= 3, fig.path = "figures/zc-sde-"}

plot_grid(tag_data17, tag_data19, tag_data20,
          labels = c('(a)', '(b)', '(c)'),
          label_size = 14, vjust = 1, hjust = -0.5,
          ncol = 3,
          rel_widths = c(1,1,1),
          align = 'hv')
```

## Figure 9: Overlaid Dive Traces + SDE Smooths

```{r, fig-9-settings}
fig9_base_size <- 12
theme_set(theme_bw(base_size = fig9_base_size) + 
            theme(text = element_text(family="Times New Roman")))
```

### Prepare Overlaid Dive Traces

#### Deep Dives

```{r, overlaid-deep, fig.height=7.5, fig.width = 6.5}
...
temp$plot$theme <- theme_bw(base_size = fig9_base_size) + 
  theme(text = element_text(family="Times New Roman"),
...
```{r, fig-9-diff-smooths, fig.height = 8.6, fig.width = 7, fig.path = "figures/zc-sde-"}
plot_grid(zc17_234a_overlaid_shallow + 
            labs(title = 'DTag17_234, Shallow Dives'),
          zc17_234a_depth_diffsmoothb,
          
          zc19_218a_overlaid_deep + 
            labs(title = 'DTag19_218, Deep Dives'),
          zc19_218a_depth_diffsmoothb,
          
          zc20_232a_overlaid_shallow + 
            labs(title = 'DTag20_232, Shallow Dives'),
          zc20_232a_depth_diffsmooth1b,
          
          zc20_232a_overlaid_deep + 
            labs(title = 'DTag20_232, Deep Dives',
                 x = 'Proportion of Dive'),
          zc20_232a_depth_diffsmooth2b,
